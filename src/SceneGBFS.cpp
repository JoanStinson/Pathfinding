...
SceneGBFS::SceneGBFS() {
	...
	gbfs = agents[0]->GBFS(pix2cell(start), coinPosition, graph);
	for (unsigned int i = 0; i < gbfs.size(); i++) {
...
void SceneGBFS::drawCoinAndStart() {
...
bool SceneGBFS::loadTextures(char* filename_bg, char* filename_bg2, char* filename_coin, char* start) {
	// Bg
