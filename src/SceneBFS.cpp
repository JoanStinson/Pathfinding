...
const char* SceneBFS::getTitle() {
	return "SDL Pathfinding Algorithms :: Breadth First Search";
}
...
Vector2D SceneBFS::cell2pix(Vector2D cell) {
	int offset = CELL_SIZE / 2;
	return Vector2D(cell.x*CELL_SIZE + offset, cell.y*CELL_SIZE + offset);
}

Vector2D SceneBFS::pix2cell(Vector2D pix) {
	return Vector2D((float)((int)pix.x / CELL_SIZE), (float)((int)pix.y / CELL_SIZE));
}

bool SceneBFS::isValidCell(Vector2D cell) {
